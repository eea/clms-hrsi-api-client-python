################################################################################
# REST API client script for batch access to Copernicus Land Monitoring Service data
################################################################################
#
...
# It foresees capabilities for search and download CLMS products (currently only the Sentinel-1 and Sentinel-2 based products of High Resolution-Snow &Ice). Users are recommended to rely on this python script, to perform custom and automatic queries.
...
def main():
    ...
    > python clms_hrsi_downloader.py output_folder -query -productType FSC -productIdentifier 31TCH -obsDateMin 2020-06-01T00:00:00Z -obsDateMax 2020-06-30T00:00:00Z\n
    > python clms_hrsi_downloader.py output_folder -hrsi_credentials hrsi_auth.txt -query_and_download -queryURL "https://cryo.land.copernicus.eu/resto/api/collections/HRSI/search.json?maxRecords=1000&publishedAfter=2021-09-05T00:00:00Z&publishedBefore=2021-09-07T00:00:00Z&productType=FSC&mission=S2"\n
    > python clms_hrsi_downloader.py output_folder -hrsi_credentials hrsi_auth.txt -download -result_file result_file.txt\n""", formatter_class=argparse.RawTextHelpFormatter)
